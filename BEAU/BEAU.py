class depletion_parms:
    ...
    def __init__(self):
        ...
        self.nMat = 5
        # this is the minimum number of times the fuel cycle must be modeled
        # before BEAU starts checking for convergence

        self.mat_weighting = .5
        # this is the weighting factor of the N+1 composition of the Nth composition
        # mat (N+1) = mat(N)*(self.mat_weighting) + mat_estimate(N+1)*(1 -
        # self.mat_weighting)
...
class equilibrium_cycle:
    ...
    def populate(self, MPO=None, cycle_length=None, neutron_source=None,):
        ...
        for circ_prog_key in self.fuel_cycle.keys():
            ...
            circ_prog.populate(title=circ_prog_key, MPO=mpo1, cycle_length=self.cycle_length[circ_prog_key], fuel_cycle=self.fuel_cycle[circ_prog_key], circuits=self.circuits[
                               circ_prog_key], depl_parms=self.depl_parms, neutron_source=self.neutron_source, burnup=self.target_burnups[circ_prog_key], pebble_flow=PF)
    ...
    def burnup(self):
        ...
        self.ave_BU = {}
        for circ_prog_key in self.fuel_cycle.keys():
            self.ave_BU[circ_prog_key] = self.circ_progs[
                circ_prog_key].burnup()
        return self.ave_BU
            ...
            for circ_key in self.fuel_cycle[circ_prog_key].keys():
                for prog_key in self.fuel_cycle[circ_prog_key][circ_key].keys():
                    for cell in self.fuel_cycle[circ_prog_key][circ_key][prog_key]:
                        ...
                        eq_mat = self.circ_progs[circ_prog_key].passes[circ_key].progressions[
                            prog_key].depletions[cell].time_ave_mat()[cell]
                        # normalize to at/bn-cm
                        eq_mat = eq_mat * (.60221415 / self.circ_progs[circ_prog_key].passes[
                                           circ_key].progressions[prog_key].depletions[cell].mpos[0].volume[cell])

                        if iteration > 1.:
                            # include previous estimate of eq mat in new
                            # estimate of eq mat
                            old = mocup.material()
                            ...
                            new = eq_mat * \
                                (1. - self.mat_weighting) + \
                                old * self.mat_weighting
                            new.make_ocf(
                                eq_ocf_loc, lib=self.depl_parms.library)

                            for nucl in input_mat.comp.keys():
                                # this loop over rides concentrations of
                                # nuclides that are depleted
                        ...
                        if cell in self.circ_progs[circ_prog_key].passes[circ_key].progressions[prog_key].depletions[cell].mpos[0].temperature.keys():
                            if self.depl_parms.transport_module[0] in 'mM':
                                mat_card = input_mat.mcf(m, tmp=self.circ_progs[circ_prog_key].passes[
                                                         circ_key].progressions[prog_key].depletions[cell].mpos[0].temperature[cell])
                        elif new.tmp != 0:
                            if self.depl_parms.transport_module[0] in 'mM':
        ...
        if self.depl_parms.transport_module[0] in 'mM':
            for token in cell_tokens:
                ...
                if cell in self.new_mats.keys():
                    ...
                    l = cell_strings[k:].find(self.mpos[cell].mat_ID[cell]) + len(
                        self.mpos[cell].mat_ID[cell])
                    n = cell_strings[k + l:].find(cell_strings[k + l:].split()[0]) + len(
                        cell_strings[k + l:].split()[0])

                    token1 = token + cell_strings[k:k + l + n]
                    new_token1 = '\n%s %s %1.5E' % (
                        cell, self.mpos[cell].mat_ID[cell], self.new_mats[cell].moles())
    ...
    def search_source(self, ):
        ...
        powers = {(1. - self.depl_parms.power_perturb) * neutron_source:
                  0, (1. + self.depl_parms.power_perturb) * neutron_source: 0}
        ...
        for NS in powers.keys():
            l += 1
            self.populate(
                MPO=self.mpo, cycle_length=self.cycle_length, neutron_source=NS)
            csv = '\n%d,%1.5E,' % (l, NS)
        ...
        while math.fabs(self.power() / self.target_power - 1.) > self.depl_parms.power_err:
            ...
            if NS < .1 * sorted(powers.keys())[0]:
                ...
                NS = 0.5 * \
                    (sorted(powers.keys())[0] + sorted(powers.keys())[-1])
...
class circuit_progression:
    ...
    def populate(self, title=None, MPO=None, cycle_length=None, neutron_source=None, fuel_cycle=None, circuits=None, depl_parms=None, burnup=None, pebble_flow=None):
        ...
        for key in self.circuits:
            ...
            circ.populate(fuel_cycle=self.fuel_cycle[key], MPO=mpo1, cycle_length=self.cycle_length * (
                circ_volumes[key] / total_volume), neutron_source=self.neutron_source, depl_parms=self.depl_parms, pebble_flow=PF)
        ...
        for i in range(0, len(self.circuits)):
            ...
            if i == 0:
                for prog in self.fuel_cycle[self.circuits[0]].keys():
                    ocf_loc = 'moi_files/moi.%s.0.pch' % self.fuel_cycle[
                        self.circuits[0]][prog][0]
                    bol = self.makeup * self.mpos[self.fuel_cycle[self.circuits[0]][prog][0]].volume[
                        self.fuel_cycle[self.circuits[0]][prog][0]]
            ...
            if i + 1 < len(self.circuits):
                ...
                # the discharge from the current circuit becomes the charge for
                # the subsequence circuit
                ...
                for prog_key in self.fuel_cycle[self.circuits[i + 1]].keys():
                    BOP_cell = self.fuel_cycle[
                        self.circuits[i + 1]][prog_key][0]
                    BOP_vol = self.passes[self.circuits[i + 1]].progressions[
                        prog_key].depletions[BOP_cell].mpos[0].volume[BOP_cell]
                    ocf_loc = 'moi_files/moi.%s.0.pch' % BOP_cell
                    charge = discharge * BOP_vol
                    charge.make_ocf(ocf_loc, lib=self.depl_parms.library)
            else:
                # there is a makeup fuel vector provided for the next circuit
                # that will be created at the beginning of this loop or the
                # discharge will not be reinserted into the core
                pass
        ...
        for circ_key in self.circuits:
            ...
            for prog_key in self.fuel_cycle[circ_key].keys():
                ...
                for cell in self.fuel_cycle[circ_key][prog_key]:
                    # note that since cycle lengths are now progression
                    # dependent I need to rederive this expression
                    prog_burnups[prog_key] += self.passes[circ_key].progressions[prog_key].time[cell] * self.passes[
                        circ_key].progressions[prog_key].ave_powers[cell] / (heavy_density * self.mpo.volume[cell])
                circ_burnup += prog_burnups[prog_key] * prog_volumes[
                    circ_key][prog_key] * self.pebble_flow[prog_key]
                circ_flow += prog_volumes[circ_key][
                    prog_key] * self.pebble_flow[prog_key]
            self.ave_BU += circ_burnup / circ_flow

        if p == 'yes':
            output = '\nCYCLE LENGTH SEARCH: PROGRESSION - %s, FLUX AMPLITUDE - %1.5E fission born neutrons per second, Target BURNUP %1.5E (MWd/MT)' % (
                self.title, self.neutron_source, self.target_burnup)
            output += '\nFUEL CYCLE INPUT PARMS:  CYCLE LENGTH - %1.5E EFPD, FLUX AMPLITUDE - %1.5E fission born neutrons per second' % (
                self.cycle_length, self.neutron_source)
            output += '\nFUEL CYCLE OUTPUT PARMS: POWER %1.5E MWth, BURNUP %1.5E MWd/MT' % (
                self.power(), self.ave_BU)
    ...
    def normalized_burnup_estimate(self):
        ...
        for circ_key in self.circuits:
            ...
            for prog_key in self.fuel_cycle[circ_key].keys():
                prog_burnups[prog_key] = 0
                for cell in self.fuel_cycle[circ_key][prog_key]:
                    prog_burnups[prog_key] += self.mpos[cell].power_density[cell] * 1.60217646e-19 * self.neutron_source * self.mpos[
                        cell].volume[cell] / (heavy_density * prog_volumes[circ_key][prog_key]) * (circ_volumes[circ_key] / total_volume)
                circ_burnup += prog_burnups[
                    prog_key] * prog_volumes[circ_key][prog_key]
        ...
        for circ_key in self.circuits:
            ...
            for prog_key in self.fuel_cycle[circ_key].keys():
                ...
                for cell in self.fuel_cycle[circ_key][prog_key]:
                    ...
                    prog_volumes[circ_key][
                        prog_key] += self.mpos[cell].volume[cell]
                    circ_volumes[circ_key] += self.mpos[
                        cell].volume[cell]
                    total_volume += self.mpos[
                        cell].volume[cell]
    ...
    def search_cycle_length(self,):
        ...
        burnups = {(1. - self.depl_parms.burnup_perturb) * cycle_length:
                   0, (1. + self.depl_parms.burnup_perturb) * cycle_length: 0}

        k = 0
        for CL in sorted(burnups.keys()):
            k += 1
            self.populate(
                title=self.title, MPO=self.mpo, cycle_length=CL, neutron_source=self.neutron_source, fuel_cycle=self.fuel_cycle,
                circuits=self.circuits, depl_parms=self.depl_parms, burnup=self.target_burnup, pebble_flow=self.pebble_flow)
            self.deplete()
            burnups[CL] = self.burnup(p='yes')
            os.system('cat scratch/DEPL.o >> scratch/cycle_length.o')
            csv = '\n%d,%1.5E,%1.5E,%1.5E,%1.5E' % (
                k, self.neutron_source, self.power(), CL, burnups[CL])
        ...
        while math.fabs(self.burnup() / self.target_burnup - 1) > self.depl_parms.burnup_err:
            ...
            # ensure residence time is reasonably larger than the maximum cycle
            # length estimate
                ...
                CL = 0.5 * \
                    (sorted(burnups.keys())[0] + sorted(burnups.keys())[-1])
...
class circuit:
    ...
    def populate(self, fuel_cycle=None, MPO=None, cycle_length=None, neutron_source=None, depl_parms=None, pebble_flow=None):
        ...
        for key in fuel_cycle.keys():
            ...
            self.cycle_lengths[
                key] = self.cycle_length * flow / pebble_flow[key]
            prog = progression()
            prog.populate(cells=fuel_cycle[key], MPO=mpo1, cycle_length=self.cycle_lengths[
                          key], neutron_source=self.neutron_source, depl_parms=self.depl_parms)
    ...
    def mix(self):
        ...
        for key in self.progressions.keys():
            ...
            total_volume += self.progressions[
                key].depletions[cell].mpos[0].volume[cell]

            mat1 = mocup.material()
            ocf_loc = 'moi_files/moi.%s.%d.pch' % (
                cell, (self.progressions[key].depletions[cell].mpos[-1].timestep + 1))
...
class progression:
    ...
    def populate(self, cells=None, MPO=None, cycle_length=None, neutron_source=None, depl_parms=None):
        ...
        for cell in self.cells:
            ...
            self.time[cell] = self.cycle_length * \
                self.mpos[cell].volume[cell] / total_volume

            # estimate the BOC mass in MT
            mat = mocup.material()
            mat.comp = self.mpos[cell].concentration[cell]
            mass = mat.heavy_mass() * self.mpos[
                cell].volume[cell] * (1e-6 / .60221415)
            # estimate the expected power in MWt
            power = self.mpos[cell].power_density[cell] * self.mpos[
                cell].volume[cell] * self.neutron_source * 1.6022e-19
            ...
            if expected_BU < self.depl_parms.nBU1 * self.depl_parms.dBU1 or expected_BU < self.depl_parms.dBU1 * self.depl_parms.nBU:
                ...
                for i in range(self.depl_parms.nBU):
                    self.depletions[cell].time.append(
                        self.time[cell] / self.depl_parms.nBU)
            else:
                # expected burnup is more than initial burnup steps (normal path)
                # generate timesteps based on not exceeding maximum burnup
                # steps

                # define timesteps for intial burnup steps
                for i in range(self.depl_parms.nBU1):
                    self.depletions[cell].time.append(
                        self.depl_parms.dBU1 * mass / power)

                expected_BU = expected_BU - \
                    self.depl_parms.dBU1 * self.depl_parms.nBU1
                nBU2 = int((expected_BU - expected_BU %
                           self.depl_parms.dBU2) / self.depl_parms.dBU2 + 1.)
    ...
    def deplete(self):
        ...
        for i in range(1, len(self.cells)):
            ...
            mat = mocup.material()
            ocf_loc = 'moi_files/moi.%s.%d.pch' % (
                self.cells[i - 1], self.depletions[self.cells[i - 1]].mpos[-1].timestep + 1)
            mat.import_ocf(ocf_loc)

            mat = mat * (self.depletions[self.cells[i]].mpos[0].volume[
                         self.cells[i]] / self.depletions[self.cells[i - 1]].mpos[0].volume[self.cells[i - 1]])
        ...
        for cell in self.cells:
            ...
            if self.depl_parms.fission_power_only == 'yes':
                ...
                for i in range(len(self.depletions[cell].time)):
                    ...
                    self.ave_powers[cell] += (self.depletions[cell].mpos[
                                              i].ORIGEN_fissionpower[cell] * self.depletions[cell].time[i])
            else:
                # use fission power and decay heat power
                for i in range(len(self.depletions[cell].time)):
                    total_time += self.depletions[cell].time[i]
                    self.ave_powers[cell] += (self.depletions[cell].mpos[
                                              i].ORIGEN_power[cell] * self.depletions[cell].time[i])
...
def origenBEAU(depletion):
    ...
    for cell in depletion.mpos[0].cell:
        ...
        # intialize moi_file directory by deleting .out, .inp and .pch files
        # for this cell
        ...
        # identify which isotopes are not depleted so these can be added back
        # into the material when the material vectors are updated
        ...
        for nucl in nuclides:
            moles = mpo.concentration[cell][
                nucl] * mpo.volume[cell] / .602214078
        ...
        for nucl in nuclides:
            ...
            # determine if fuel or fission product and which library to append
            # to and cross section format
            ...
            # initialize fraction of NG XS and N2N XS that end in ground state
            ...
            # grab cross sections from XS library to allocate the NG  and N2N
            # cross section to reactions that end in ground state and
            # metastable states
            if NXS_lib[k + 14:k + 16] != '  ':
                NG = float(NXS_lib[k + 7:k + 12]) * math.pow(
                    10, int(NXS_lib[k + 13:k + 16]))
            if NXS_lib[k + 24:k + 26] != '  ':
                N2N = float(NXS_lib[k + 17:k + 22]) * math.pow(
                    10, int(NXS_lib[k + 23:k + 26]))
            if NXS_lib[k + 54:k + 56] != '  ':
                NGex = float(NXS_lib[k + 47:k + 52]) * math.pow(
                    10, int(NXS_lib[k + 53:k + 56]))
            if NXS_lib[k + 64:k + 66] != '  ':
                N2Nex = float(NXS_lib[k + 57:k + 62]) * math.pow(
                    10, int(NXS_lib[k + 63:k + 66]))
        ...
        for i in range(len(depletion.time)):
            if commands + 13 < 300 and irradiations + 10 < 150:
                ...
                # the input exceeds one of the command limits so a new input
                # must be created
                commands = 12
                irradiations = 0
                TAPE5 += footer
            ...
            if depletion.mode == 'power':
                ...
                if depletion.mode == 'source':
                    power_string = '%1.5E' % (
                        depletion.source[i] * mpo.flux_tally[cell])
                else:
                    power_string = '%1.5E' % (depletion.power[i] * mpo.flux_tally[
                                              cell] / (mpo.global_power_density * 1.60217464e-19))
        ...
        for index in sorted(TAPE5s.keys()):
            ...
            if depletion.library[0:3] in ['amo', 'emo', 'fft']:
                ...
                # origen uses a thermal reactor set of cross sections, thus
                # origen will be executed with o2_therm
                origen = 'o2_therm'
            ...
            ORIGEN_powers = re.compile(
                'SP POW,MW\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+').findall(TAPE6)
            ORIGEN_fluxes = re.compile(
                'NEUT. FLUX\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+').findall(TAPE6)
            ...
            # split TAPE7 into multiple PCH files, recover part that I 'split'
            # out
            zeros = re.compile(
                '\s{3,3}0\s{7,7}\d[.]\d{5,5}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{5,5}[eE+-]{2,2}\d{2,2}\s+\d[.]\d{2,2}[eE+-]{2,2}\d{2,2}\s+').findall(TAPE7)
            ...
            for jndex in range(len(ORIGEN_powers)):
                ...
                if len(_STRUCTURAL_) > 0:
                    ...
                    for j in range(3, len(ORIGEN_powers[jndex].split())):
                        ORIGEN_power += float(
                            ORIGEN_powers[jndex].split()[j]) / 10.
                        ORIGEN_decayheat += 1e-6 * \
                            float(decay_heat[i][j - 3]) / 10.
                        ORIGEN_decayheat += 1e-6 * \
                            float(decay_heat[i + 1][j - 3]) / 10.
                        ORIGEN_decayheat += 1e-6 * \
                            float(decay_heat[i + 2][j - 3]) / 10.
                ...
                depletion.mpos[index + jndex].ORIGEN_power[
                    cell] = ORIGEN_power + ORIGEN_decayheat
                depletion.mpos[index + jndex].ORIGEN_fissionpower[
                    cell] = ORIGEN_power
                depletion.mpos[index + jndex].ORIGEN_fluxes[
                    cell] = ORIGEN_flux
    ...
    def equilibriate(self, depl_parms):
        ...
        if self.converged == 'no':
            ...
            if depl_parms.transport_module[0] in 'mM':
                ...
                self.li6_ratio.append(be9_tally[be9_tally.keys()[0]][be9_tally[be9_tally.keys()[0]].keys()[0]] / li6_tally[
                                      li6_tally.keys()[0]][li6_tally[li6_tally.keys()[0]].keys()[0]])
            ...
            for li_mat in self.li_mats:
                ...
                if '40090' in mat.comp.keys() and '30060' in mat.comp.keys():
                    ...
                    if len(self.li6_ratio) > 1:
                        li6_ratio = (1. - self.mat_weighting) * self.li6_ratio[
                            -1] + self.mat_weighting * self.li6_ratio[-2]
                    else:
...
def search_comp(eq_fuel_cycle):
    ...
    while 'Material Converged' not in comp_convergence(eq_fuel_cycle.depl_parms, keff_list, mat_list):
        ...
        csv = '%1.5E,%s' % (keff(eq_fuel_cycle.depl_parms), comp_convergence(
            eq_fuel_cycle.depl_parms, keff_list, mat_list))
        open('scratch/comp.csv', 'a').write(csv)
...
def search_burnup(eq_fuel, burnup=None):
    ...
    # search for the burnup of the driver circuit progress to impose a target
    # power
    if burnup == None:
        ...
        BU = burnup
    keffs = {(1. - eq_fuel.depl_parms.burnup_perturb) * BU:
             0, (1. + eq_fuel.depl_parms.burnup_perturb) * BU: 0, }
...
def transportBRO(depl_parms, index=None):
    ...
    if index == None:
        ...
        scratch_indexes = open('tmp').read().replace(
            ' ', '').replace('\n', '').split('backup/inp')
...
def least_squares(y_vector):
    ...
    slope = (sum_xy - 1. / len(y_vector.values()) * sum_x * sum_y) / \
        (sum_x2 - 1. / len(y_vector.values()) * sum_x ** 2)
    x = y_vector.keys()[0]
    intercept = sum_y / \
        len(y_vector.values()) - sum_x / len(y_vector.values()) * slope
...
def output(eq_fuel_cycle):
    ...
    for circ_prog_key in eq_fuel_cycle.fuel_cycle.keys():
        ...
        for nucl in eq_fuel_cycle.makeup[circ_prog_key].comp.keys():
            output += '\n%s.comp[%s] = %1.5E' % (
                mat_ID, nucl, eq_fuel_cycle.makeup[circ_prog_key].comp[nucl])
    ...
    for circ_prog_key in eq_fuel_cycle.fuel_cycle.keys():
        for circ_key in eq_fuel_cycle.circuits[circ_prog_key]:
            for prog_key in eq_fuel_cycle.fuel_cycle[circ_prog_key][circ_key].keys():
                for cell in eq_fuel_cycle.fuel_cycle[circ_prog_key][circ_key][prog_key]:
                    output += "\n'" + cell + "':\n\t"
                    output += '%1.5E,' % eq_fuel_cycle.circ_progs[circ_prog_key].passes[
                        circ_key].progressions[prog_key].ave_powers[cell]
    output += '\n}'
        ...
        for circ_key in eq_fuel_cycle.circuits[circ_prog_key]:
            for prog_key in eq_fuel_cycle.fuel_cycle[circ_prog_key][circ_key].keys():
                for cell in eq_fuel_cycle.fuel_cycle[circ_prog_key][circ_key][prog_key]:
                    output += "\n'" + cell + "':\n\t"
                    output += '%1.5E,' % (
                        eq_fuel_cycle.neutron_source * eq_fuel_cycle.mpo.flux_tally[cell])
    output += '\n}'
    ...
    for circ_prog_key in eq_fuel_cycle.fuel_cycle.keys():
        output += "'%s':%1.5E," % (
            circ_prog_key, eq_fuel_cycle.circ_progs[circ_prog_key].burnup())
    output += '\n}'
        ...
        for circ_key in eq_fuel_cycle.circuits[circ_prog_key]:
            for prog_key in eq_fuel_cycle.fuel_cycle[circ_prog_key][circ_key].keys():
                for cell in eq_fuel_cycle.fuel_cycle[circ_prog_key][circ_key][prog_key]:
                    output += "\n'" + cell + "':\n\t"
                    output += '%2.1f,' % (
                        100 * (1. - eq_fuel_cycle.new_mats[cell].heavy_moles() / a.heavy_moles()))
    output += '\n}'
    ...
    for circ_prog_key in eq_fuel_cycle.fuel_cycle.keys():
        output += "'%s':%1.5E" % (
            circ_prog_key, eq_fuel_cycle.cycle_length[circ_prog_key])
    output += '\n}'
    ...
    for circ_prog_key in eq_fuel_cycle.fuel_cycle.keys():
        for circ_key in eq_fuel_cycle.circuits[circ_prog_key]:
            for prog_key in eq_fuel_cycle.fuel_cycle[circ_prog_key][circ_key].keys():
                for cell in eq_fuel_cycle.fuel_cycle[circ_prog_key][circ_key][prog_key]:
                    output += "\n'" + cell + "':\n\t"
                    output += '%1.5E,' % eq_fuel_cycle.circ_progs[
                        circ_prog_key].passes[circ_key].progressions[prog_key].time[cell]
    output += '\n}'
                ...
                for nucl in nuclides:
                    ...
                    for cell in eq_fuel_cycle.fuel_cycle[circ_prog_key][circ_key][prog_key]:
                        for i in range(len(eq_fuel_cycle.circ_progs[circ_prog_key].passes[circ_key].progressions[prog_key].depletions[cell].time) + 1):
                            ...
                            mat = mat * \
                                (.60221415 / eq_fuel_cycle.mpo.volume[cell])
                            if nucl in mat.comp.keys():
                                mat_out += '%1.5E,' % mat.comp[nucl]
                            else:
                                mat_out += '%1.5E,' % 0

                mat_loc = 'concentration.%s.%s.%s.csv' % (
                    circ_prog_key, circ_key, prog_key)
